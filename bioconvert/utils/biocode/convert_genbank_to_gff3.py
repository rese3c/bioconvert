"""
TC 2022: Code from biocode copy/paste in bioconvert while waiting for biocode to be in bioconda
...
def gbk2gff3(infile, outfile=None, fasta=False):
    ...
    for gb_record in SeqIO.parse(open(infile, "r"), "genbank"):
        ...
        if len(gb_record.seq) > 0:
            seqs_pending_writes = True
            try:
                assemblies[mol_id].residues = str(gb_record.seq)
                assemblies[mol_id].length = len(gb_record.seq)
            except UndefinedSequenceError:
                assemblies[mol_id].residues = "undefined"
                assemblies[mol_id].length = len(gb_record.seq)
